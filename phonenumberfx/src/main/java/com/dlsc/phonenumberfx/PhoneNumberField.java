...
public class PhoneNumberField extends Control {
    ...
    private static final Map<CountryCallingCode, Image> FLAG_IMAGES = new HashMap<>();

    static {
        for (CountryCallingCode code : CountryCallingCode.Defaults.values()) {
            FLAG_IMAGES.put(code, new Image(Objects.requireNonNull(PhoneNumberField.class.getResource("phonenumberfield/country-flags/" + code.iso2Code().toLowerCase() + ".png")).toExternalForm()));
        }
    }
